...
public record UpdateExitSlipActiveStatusCommand(UpdateExitSlipActiveStatusRequest UpdateExitSlipActiveStatusRequest)
...
{
    async Task<Result<bool>> IRequestHandler<UpdateExitSlipActiveStatusCommand, Result<bool>>.Handle(
        UpdateExitSlipActiveStatusCommand request, CancellationToken cancellationToken)
    {
        try
        {
            // Load
